classdef Plotter
    ...
    methods (Static)
        function plotTrajectory(x_true, x_est, P_history, scenario_name, filter_name, plot_folder)
            % Skip existing plot; otherwise create folder and render
            filename = fullfile(plot_folder, sprintf('trajectory_%s_%s.png', scenario_name, filter_name));
            if exist(filename, 'file')
                return;
            end
            PathHelper.ensurePath(plot_folder, '', '', true);
        ...
        function plotCovarianceEllipse(mu, Sigma, nsigma)
            % Draw nsigma ellipse for 2D covariance
            [V, D] = eig(Sigma);
        ...
        function plotNEES(nees_mean, nx, scenario_name, filter_name, plot_folder)
            % Mean NEES with chi-square bounds
            filename = fullfile(plot_folder, sprintf('nees_%s_%s.png', scenario_name, filter_name));
            if exist(filename, 'file')
                return;
            end
            PathHelper.ensurePath(plot_folder, '', '', true);
        ...
        function plotErrorHistogram(errors, scenario_name, filter_name, plot_folder)
            % Positional error magnitude histogram
            filename = fullfile(plot_folder, sprintf('error_hist_%s_%s.png', scenario_name, filter_name));
            if exist(filename, 'file')
                return;
            end
            PathHelper.ensurePath(plot_folder, '', '', true);
